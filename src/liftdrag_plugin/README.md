...
> Note: In AVL, control derivatives (such as ailerons, elevators, and rudders) may have a conversion relationship with the $\delta_{ctrl,x}$ here, as mentioned in section 4.2.1 "Control Surfaces" of the book [Small Unmanned Aircraft: Theory and Practice](https://github.com/byu-magicc/mavsim_public). The example here is actually a standard aircraft configuration. For flying-wing or tailsitter configurations, there may be implicit conversion relationships that need to be taken into account. The effectiveness of `tools/simulation/gz/tools/avl_automation/input_avl.py` for these special configurations is yet to be verified.
...
A sigmoid function was used because $C_D$ initially increases quickly as aspect ratio rises, but that increase would slow down as AR goes to infinity. Intuitively, it seemed like a good model for the drag vs. aspect ratio relation, and the fit seemed pretty good as well.
Looking at the data from Stringer, one sees that the $C_D$ was approximately equal to $1-cos(2α)$. This has a maximum value of 2, so this expression was divided by 2 to make sure multiplying it by flat-plate drag would not produce unreasonable results.
